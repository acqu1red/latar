...
- Шкафчики или полки`;
    ...
    return `Проанализируй фото комнаты для создания 2D плана квартиры. Найди основную мебель и верни JSON.

Используй типы объектов:
[bed, sofa, chair, table, wardrobe, stove, fridge, sink, toilet, bathtub, shower, washing_machine, kitchen_block]

ВАЖНО - найди хотя бы 1-3 основных объекта если они есть на фото:
- Кровать (bed) - если видна кровать
- Диван (sofa) - если видна мягкая мебель
- Стол (table) - если виден стол
- Стул (chair) - если видны стулья
- Кухонная техника (stove, fridge, sink) - если это кухня
- Сантехника (toilet, bathtub, shower, sink) - если это ванная
- Шкаф (wardrobe) - если видны шкафы

Координаты x,y (0-1) — центр объекта, w,h (0-1) — размер относительно комнаты.

ОБЯЗАТЕЛЬНО добавь двери и окна:
- doors: [{ side: "left/right/top/bottom", pos: 0.5 }]
- windows: [{ side: "left/right/top/bottom", pos: 0.5, len: 0.3 }]

${roomSpecificInstructions}

Верни JSON с найденными объектами.
Комната: ${name}, ${sqm} м².`;
};
