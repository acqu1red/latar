...
export default function MonochromeClaudeStyle() {
  ...
  const nudgeAttachHint = () => {
    if (hintTimerRef.current) clearTimeout(hintTimerRef.current);
    setAttachHint(true);
    hintTimerRef.current = setTimeout(() => setAttachHint(false), 2400);
  };
  ...
  // Left drawer & chats modal
  const [leftOpen, setLeftOpen] = useState(false);
  const [leftTab, setLeftTab] = useState("history"); // 'history' | 'settings'
  ...
  const send = () => {
    ...
    let content = value;
    if (isRemove) {
      content = `Удаление объектов — ${
        removeDepth === "surface" ? "Поверхностно" : removeDepth === "partial" ? "Частично" : "Полностью"
      }`;
    } else if (isPlan) {
  ...
  return (
    <div data-style={siteStyle} className="min-h-screen themed-root antialiased relative overflow-hidden">
      {/* Liquid background only for VENOM (extra dim) */}
      <LiquidBackdrop enabled={siteStyle === "venom"} dim={0.75} />
      <VenomDNA enabled={siteStyle === "venom"} amount={3} opacity={0.12} scale={0.9} />

      <div className="flex h-screen relative z-10">
        {/* ==== Left rail ==== */}
        <aside className="w-16 shrink-0 theme-border border-r theme-panel backdrop-blur flex flex-col items-center justify-between py-4">
          <div className="flex flex-col items-center gap-4">
            {/* По клику на панель — открываем Список чатов (модал) */}
            <button onClick={() => setChatsModalOpen(true)} className="h-10 w-10 grid place-items-center rounded-xl hover:bg-black/10" title="Список чатов">
            ...
            {/* Убрали прежний значок списка чатов */}
            ...
            <button ref={settingsBtnRef} onClick={() => openLeft("settings")} className="h-10 w-10 grid place-items-center rounded-xl hover:bg-black/10" title="Настройки">
        ...
        {/* ==== Main content ==== */}
        <main className="flex-1 overflow-y-auto flex flex-col justify-center items-center">
          <div className="mx-auto max-w-3xl px-6">
            ...
            <div className="mt-6 text-center">
              <motion.h1
                ...
                className="text-2xl sm:text-3xl font-semibold tracking-tight leading-tight font-hero"
              >
                <TwinkleStar />
                {user ? `Приветствую, ${user.name}` : "Приветствую, удачной работы!"}
              </motion.h1>
            </div>
                      ...
                      {m.attachments?.length > 0 && (
                        <div className="mt-2 grid grid-cols-3 gap-1">
                          {m.attachments.map((att) => (
                            <img key={att.id} src={att.url} alt={att.name} className="h-20 w-full object-cover rounded-lg theme-border"/>
                          ))}
                        </div>
                      )}
            ...
            <div className="mt-8 flex">
              <div className={cn("w-full max-w-2xl rounded-2xl backdrop-blur relative theme-panel theme-border","shadow-[0_0_0_1px_rgba(255,255,255,0.03)_inset]") }>
                <div className="p-3 sm:p-4">
                  <div className="flex items-start gap-3">
                    ...
                    {isPlan ? (
                      <div className="flex w-full items-center gap-2">
                        <button
                          onClick={() => { setPlanFurniture("with"); if (attachments.length === 0) nudgeAttachHint(); } }
                          className={cn(
                        ...
                        <button
                          onClick={() => { setPlanFurniture("without"); if (attachments.length === 0) nudgeAttachHint(); } }
                          className={cn(
                            "rounded-xl border px-3 py-2 text-sm",
                      ...
                      <div className="flex w-full items-center gap-2">
                        <button
                          onClick={() => { setRemoveDepth("surface"); if (attachments.length === 0) nudgeAttachHint(); } }
                          className={cn("rounded-xl px-3 py-2 text-sm border", removeDepth === "surface" ? "accent-button" : "theme-panel theme-border")}
                        >
                          Поверхностно
                        </button>
                        <button
                          onClick={() => { setRemoveDepth("partial"); if (attachments.length === 0) nudgeAttachHint(); } }
                          className={cn("rounded-xl px-3 py-2 text-sm border", removeDepth === "partial" ? "accent-button" : "theme-panel theme-border")}
                        >
                          Частично
                        </button>
                        <button
                          onClick={() => { setRemoveDepth("full"); if (attachments.length === 0) nudgeAttachHint(); } }
                    ...
                    <div className="flex shrink-0 items-center gap-2 self-end pb-1 relative">
                      <div className="relative">
                        ...
                        {showModelMenu && (
                          <div className="absolute right-0 top-10 z-20 w-56 rounded-xl theme-border theme-panel shadow-lg">
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          canSend ? "accent-button" : "disabled-button"
                        )}
                        title="Отправить"
                      >
                  ...
                  { ((isRemove || isPlan) && attachHint && attachments.length === 0) && (
      ...
      {leftOpen && (
        <>
          <div className="fixed inset-0 bg-black/40" onClick={() => setLeftOpen(false)} />
          <motion.aside
            initial={{ x: -360 }}
            animate={{ x: 0 }}
            exit={{ x: -360 }}
            transition={{ type: "spring", stiffness: 260, damping: 30 }}
            className="fixed left-0 top-0 h-full w-[360px] theme-border theme-panel shadow-2xl flex flex-col z-20"
          >
            <div className="flex items-center justify-between px-4 h-14 border-b theme-border">
              <div className="text-sm theme-text-muted">{leftTab === 'history' ? 'История чатов' : 'Настройки'}</div>
              <button onClick={() => setLeftOpen(false)} className="h-8 w-8 grid place-items-center rounded-lg hover:opacity-90"><X className="h-4 w-4" /></button>
            </div>

            {leftTab === 'history' && (
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                <button onClick={createChat} className="w-full rounded-xl theme-border theme-panel px-3 py-2 text-sm hover:opacity-90 flex items-center gap-2"><Plus className="h-4 w-4"/>Новый чат</button>
                <div className="text-xs theme-text-muted">Всего: {chats.length}</div>
                <div className="space-y-2">
                  {chats.map(c => (
                    <button key={c.id} onClick={() => { setActiveChatId(c.id); setLeftOpen(false); }} className={cn("w-full text-left rounded-xl border px-3 py-2 hover:opacity-95", c.id === activeChatId ? "theme-panel" : "theme-panel-muted", "theme-border") }>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-6">
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    <input className="w-full rounded-xl theme-border theme-panel px-3 py-2 outline-none" placeholder="you@example.com" />
                    <label className="block text-xs theme-text-muted mt-3">Пароль</label>
                    <input type="password" className="w-full rounded-xl theme-border theme-panel px-3 py-2 outline-none" placeholder="••••••••" />
      ...
      {chatsModalOpen && (
        <div className="fixed inset-0 z-30 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/50" onClick={() => setChatsModalOpen(false)} />
          <div className="relative w-full max-w-lg rounded-2xl theme-border theme-panel p-4 shadow-xl">
            <div className="flex items-center justify-between mb-3">
              <div className="text-sm theme-text-muted">Все чаты</div>
              <button onClick={() => setChatsModalOpen(false)} className="h-8 w-8 grid place-items-center rounded-lg hover:opacity-90"><X className="h-4 w-4"/></button>
            </div>
            <div className="space-y-2 max-h-[60vh] overflow-y-auto">
              {chats.map(c => (
                <button key={c.id} onClick={() => { setActiveChatId(c.id); setChatsModalOpen(false); }} className={"w-full text-left rounded-xl theme-border theme-panel px-3 py-2 hover:opacity-95"}>
                  <div className="text-sm truncate">{c.title || 'Без названия'}</div>
                  <div className="text-[11px] theme-text-muted">Сообщений: {c.messages.length}</div>
                </button>
              ))}
